...
function Shape(type, values, option) {
  var enableTypes = [
    'line',
    'rect',
    'circle'
...
Shape.prototype.draw = function(context, img, drawBounding) {
  ...
  drawToCanvas(context,
    this.getValue('type'),
    this.getValue(['width', 'height', 'coordinates', 'scaleX', 'scaleY', 'radian', 'drawTranslate', 'drawGapX', 'drawGapY']),
...
function drawLine(context, width, height, coordinates) {
  var w = (coordinates.x2 - coordinates.x1) / 2
    , h = (coordinates.y2 - coordinates.y1) / 2;
  context.moveTo(-w, -h);
  context.lineTo(w, h);
}
